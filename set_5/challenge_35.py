...
class MITM:
    def __init__(self, alice: DiffieHellmanSide, bob: DiffieHellmanSide):
        self._alice = alice
        self._bob = bob
        self._bob.set_private_key(self._alice.get_public_key())
        self._alice.set_private_key(self._bob.get_public_key())
        self._key = 1 if (self._alice.get_public_key() == 1 or self._bob.get_public_key()
                          == 1) else (DiffieHellmanSide.p - 1)
...
def main():
    alice = DiffieHellmanSide(DiffieHellmanSide.p - 1)
    bob = DiffieHellmanSide(DiffieHellmanSide.p - 1)
    mitm = MITM(alice, bob)
