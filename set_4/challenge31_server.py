...
def sha1(data):
    ...
    Returns:
        A byte SHA-1 digest of the input message.
    """
...
def hmac_sha1(key, data):
    if len(key) > 64:
        key = Sha1Hash().update(key).digest()
    if len(key) < 64:
        key += b'\0' * (64 - len(key))
    return sha1(xor_with_key(key, b'\x5c') + sha1(xor_with_key(key, b'\x36')))
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        data = parse_url_data(self.path[self.path.find('?') + 1:])
        f = data['file']
        signature = int(data['signature'], 16).to_bytes(20, 'big')
        file_signature = hmac_sha1(key, f)
        print(file_signature.hex())
        if len(signature) != len(file_signature):
            self.send_response_only(500)
            self.end_headers()
            return

        for i in range(len(file_signature)):
            if signature[i] != file_signature[i]:
                self.send_response_only(500)
                self.end_headers()
                return
            time.sleep(0.005)
