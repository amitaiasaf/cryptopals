...
class Sha1Hash(object):
    ...
    def __init__(self, h=None, processed=0):
        ...
        self._message_byte_length = processed
...
def sha1(data, h=None, processed=0):
    ...
    """
    return Sha1Hash(h, processed).update(data).hexdigest()
...
def get_message_signature(message):
    return sha1(SECRET_KEY + message)


def verify_message_signature(message, signature):
    assert get_message_signature(message) == signature
...
def main():
    ...
    h = tuple(int(signature[i:i+8], 16) for i in range(0, len(signature), 8))
    new_signature = sha1(b";admin=true", h, len(pad_message(message, len(SECRET_KEY))) + len(SECRET_KEY))
